# PetFit - 반려동물 건강 관리 서비스

## 🐾 서비스 소개

반려동물의 건강 루틴, 특이사항, 알람을 한 곳에서 관리할 수 있는 `반려동물 건강 관리 서비스`입니다. 사용자는 하루 일정과 건강 기록을 직관적으로 확인하고, 반복 루틴과 특이사항을 편리하게 관리할 수 있습니다.
...
![서버 상태 단일 진실원](./docs/images/issue-ssot-and-ui-edit-state.png)
...
![폼 검증 책임 단일화](./docs/images/issue-form-validation-separation.png)
...
초기에는 기능 구현에 집중하면서 결합도가 높아졌고, 작은 수정에도 전체 코드에 영향을 미치는 구조가 되었습니다. 이를 계기로 디자인 시스템 도입 이전에 Form/Input 구조를 책임 기준으로 정리하며 재사용 가능하고 테스트와 재현이 쉬운 컴포넌트 구조를 구축할 수 있었습니다.

또한 협업 과정에서 RFC와 ADR를 도입해 구조 변경과 기술 선택을 사전에 논의/합의하는 경험을 했고, 의사결정의 맥락을 기록하고 공유하는 협업 방식의 중요성을 체감했습니다.

마지막으로 MVP 1.0 이후 약 40명 규모의 실제 사용자 피드백을 통해 개발자가 의도한 흐름과 사용자가 이해한 흐름이 다를 수 있음을 인지했습니다. 이를 바탕으로 인증·입력 과정에 가이드 요소를 추가하며 초기 온보딩 경험을 개선할 수 있었습니다.
